# API Contract: Obtener Catálogo de Bases de Datos
...
Endpoint público que retorna el catálogo completo de bases de datos disponibles en el sistema. Permite a los trabajadores de la DGTIC seleccionar dinámicamente la base de datos sobre la cual solicitan acceso al llenar el formato de BD.
...
**Rationale**: La lista de bases de datos disponibles no es información sensible y debe ser accesible para que los clientes puedan configurarse antes de autenticarse (mismo criterio que `/api/v1/admin/ambientes-desarrollo`).
...
|-------|------|----------|-------------|
| `data` | `array` | Yes | Array de objetos de base de datos |
| `data[].id` | `integer` | Yes | Identificador único de la base de datos (> 0) |
| `data[].nombre` | `string` | Yes | Nombre/código de la base de datos |
...
- Si no hay bases de datos configuradas/activas, `data` es array vacío `[]`
- `data[].id` es siempre un entero positivo
- `data[].nombre` nunca está vacío
- "OTROS" se devuelve como una fila más del catálogo (`id` + `nombre`), sin campo adicional de texto libre — esa capacidad está fuera de alcance (ver `spec.md`)

**Empty Response Example** (cuando no hay bases de datos configuradas):
...
```bash
curl -X GET "https://api.sader.gob.mx/api/v1/admin/bases-datos" \
...
```javascript
fetch('https://api.sader.gob.mx/api/v1/admin/bases-datos', {
  method: 'GET',
  headers: {
  ...
  .then(data => {
    if (data.success) {
      console.log('Bases de datos disponibles:', data.data);
    } else {
...
try {
    $response = $client->get('/api/v1/admin/bases-datos', [
    ...
    if ($body['success']) {
        foreach ($body['data'] as $baseDatos) {
            echo "ID: {$baseDatos['id']}, Nombre: {$baseDatos['nombre']}\n";
        }
    }
} catch (\Exception $e) {
...
response = requests.get(
    'https://api.sader.gob.mx/api/v1/admin/bases-datos',
    headers={'Accept': 'application/json'}
)
...
if response.status_code == 200:
    data = response.json()
    if data['success']:
        for base_datos in data['data']:
            print(f"ID: {base_datos['id']}, Nombre: {base_datos['nombre']}")
else:
...
**Rationale**: Es un endpoint de consulta simple sin carga computacional significativa, y se espera bajo volumen de peticiones (mismo criterio que `ambientes-desarrollo`).
...
- **Strategy**: Los datos vienen de PostgreSQL pero son muy estáticos (4 registros raramente cambian)
- **Current**: Sin caching implementado (query directo a DB en cada request)
- **Future consideration**: Si la carga aumenta, considerar cache en Redis con TTL largo (1 hora+) e invalidación manual al actualizar bases de datos
- **Performance**: Query es muy ligero (4 registros, índice en `ind_activo`), performance adecuada sin cache
...
- Las bases de datos raramente cambian, caching agresivo es seguro
...
- Agregar nuevas bases de datos a la lista
- Cambiar mensajes de error (siempre que el formato se mantenga)

## Backward Compatibility Guarantees

**Guaranteed stable**:
- URL path `/api/v1/admin/bases-datos`
- HTTP method `GET`
...
- Número de bases de datos en la lista
...
- [ ] `code` es "200" (string)
- [ ] `data` contiene únicamente bases de datos con `ind_activo = 1`
- [ ] POST/PUT/DELETE retornan 405
- [ ] Respuesta con catálogo vacío retorna `data: []`
- [ ] Response cumple con JSON schema definido

### Example PHPUnit Contract Test

```php
public function test_contrato_obtener_bases_datos(): void
{
    $response = $this->getJson('/api/v1/admin/bases-datos');
    ...
    foreach ($data as $baseDatos) {
        $this->assertIsInt($baseDatos['id']);
        $this->assertGreaterThan(0, $baseDatos['id']);
        $this->assertIsString($baseDatos['nombre']);
...
**Potential threats**:
- Information disclosure: **Acceptable** - los nombres de bases de datos no son sensibles
...
Los nombres/códigos de bases de datos (e.g., "PPB", "SURI", "XAMAN", "OTROS") no constituyen información sensible o confidencial.
...
```json
{
  ...
  "level": "INFO",
  "message": "Bases de datos obtenidas",
  "context": {
    "endpoint": "/api/v1/admin/bases-datos",
    "method": "GET",
    "status": 200,
    "response_time_ms": 15,
    "base_datos_count": 4,
...
|---------|------|---------|
| v1.0 | 2026-08-07 | Initial contract definition |
