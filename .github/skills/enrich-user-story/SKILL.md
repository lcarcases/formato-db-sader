...
When drafting the final requirement, you MUST:

1. **Create directory structure**: Ensure `userStory/enriched/` directory exists in the workspace root
2. **Generate filename**: Use the format `{date}-{sanitized-title}-user-story.md` where:
   - `{date}` is the current date in YYYY-MM-DD format
   - `{sanitized-title}` is the requirement title in kebab-case (lowercase, spaces replaced with hyphens, special characters removed)
   - Example: `2026-05-02-create-user-authentication-endpoint-user-story.md`
3. **Save the markdown file**: Write the complete enriched requirement to the file path `userStory/enriched/{filename}.md`
4. **Confirm to user**: After saving, inform the user of the file location
...
**Step 1**: Create the markdown file in `userStory/enriched/` with the following structure:
...
**Step 2**: Save the file to `userStory/enriched/{date}-{sanitized-title}.md`

**Step 3**: Respond to the user in the same language:

✅ **Enriched user story created**

**File**: `userStory/enriched/{filename}.md`

The requirement has been documented and is ready for Spec-Driven Development (SDD) planning.

**Next steps:**
- Review the enriched requirement
- Use it as input for `/speckit.specify`
