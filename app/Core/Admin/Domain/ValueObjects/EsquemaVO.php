...
/**
 * Value Object representing an Esquema
 *
 * Immutable domain concept representing a database schema catalog entry to
 * which a DGTIC worker may request access within a given hostname. Identity
 * is defined by its attributes (not by reference).
 *
 * Domain invariants:
 * - ID must be a positive integer (> 0)
 * - Nombre must not be empty after trimming
 *
 * Note: The synthetic "Todos" entry (id = 0) never goes through this class —
 * the id > 0 invariant is never relaxed.
 */
final readonly class EsquemaVO
{
    /**
     * Create a new Esquema Value Object
     *
     * @param  int  $id  Identificador único del esquema (must be > 0)
     * @param  string  $nombre  Nombre del esquema de base de datos (non-empty)
     *
    ...
    /**
     * Create Esquema from array (useful for mapping from database/config)
     *
